...
เมื่อ Gemini ตอบ `429` Worker จะสลับโมเดลโดยอัตโนมัติตามลำดับ `GEMINI_MODEL` → `gemini-2.5-flash` → `gemini-2.5-flash-lite` → `gemini-3.5-flash` โดยตัดชื่อที่ซ้ำออก ข้อผิดพลาดชนิดอื่นจะไม่สลับโมเดลเพื่อป้องกันการส่งคำขอซ้ำโดยไม่จำเป็น หากทุกโมเดลเต็ม API จะตอบ `503` พร้อมรหัส `AI_CAPACITY_EXHAUSTED` และลิงก์สนับสนุนจาก `SUPPORT_URL`
