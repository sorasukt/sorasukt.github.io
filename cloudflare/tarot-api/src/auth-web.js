...
async function finishLogin(request, env) {
  ...
  if (url.searchParams.get("state") !== transaction.state) {
    return authFailure(transaction.returnTo, "Authentication state mismatch", clearTx);
  }

  if (url.searchParams.has("error")) {
    return authFailure(transaction.returnTo, url.searchParams.get("error_description") || url.searchParams.get("error") || "Authentication failed", clearTx);
  }
  ...
  if (!tokenResponse.ok) {
    return authFailure(transaction.returnTo, "Unable to exchange authorization code", clearTx);
  }
...
async function verifyIdToken(token, env, expectedNonce) {
  ...
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    if (payload.iss !== issuer || payload.aud !== env.AUTH0_CLIENT_ID || payload.exp <= now || payload.iat > now + 60 || payload.nonce !== expectedNonce || !payload.sub) return null;
...
function auth0Domain(env) {
  return (env.AUTH0_DOMAIN || "auth.sorasukt.com").replace(/^https?:\/\//, "").replace(/\/$/, "");
}

function assertConfig(env) {
  if (!env.AUTH0_CLIENT_ID || !env.AUTH0_CLIENT_SECRET) throw new Error("Auth0 Regular Web Application is not configured");
}
