...
async function getDaily(env,headers,sub,deck){
  ...
  try{const {result:horoscope}=await generateGeminiJson(env,{system:"You create concise, grounded daily reflective horoscope guidance in natural Thai. Treat astrology and Tarot as reflective frameworks, not factual prediction. Never claim certainty.",prompt:`Create today's personalized daily reflection for ${day}. Birth date: ${p.birth_date}. Birth time: ${p.birth_time||"not provided"}. Birth place: ${p.birth_place||"not provided"}. Daily Tarot card: ${card.name}.`,schema:DAILY_SCHEMA});await env.DB.prepare("UPDATE daily_readings SET status='ready',horoscope_json=?,updated_at=CURRENT_TIMESTAMP WHERE user_sub=? AND reading_date=?").bind(JSON.stringify(horoscope),sub,day).run();return json({success:true,cached:false,date:day,card:{id:card.id,name:card.name},horoscope},200,headers)}catch(error){await env.DB.prepare("DELETE FROM daily_readings WHERE user_sub=? AND reading_date=? AND status='pending'").bind(sub,day).run();if(error instanceof GeminiCapacityError)return json({success:false,error:capacityError(env)},503,headers);const t=error?.name==="AbortError";return json({success:false,error:{code:t?"AI_TIMEOUT":"AI_GENERATION_FAILED",message:"ไม่สามารถสร้างดวงประจำวันได้ในขณะนี้"}},t?504:502,headers)}
}
