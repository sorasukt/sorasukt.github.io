(() => {
  ...
  async function hydrateFromMember(){
    try{
      ...
      if(member?.profile?.birth_date&&!$('#astroBirthDate').value)$('#astroBirthDate').value=member.profile.birth_date;
    }catch{}
  }

  $('#astroForm').addEventListener('submit',e=>{e.preventDefault();const d=new Date($('#astroBirthDate').value+'T00:00:00'),z=zodiac(d.getDate(),d.getMonth()+1);$('#astroResult').hidden=false;$('#astroResult').innerHTML=`<h2>ราศีอาทิตย์: ${z.name}</h2><p>${z.copy}</p><p>ภาพรวมนี้ใช้วันเกิดเท่านั้น หากคุณเป็นสมาชิก ระบบสามารถใช้ข้อมูลเกิดที่บันทึกไว้เพื่อเปิดการอ่านเชิงลึกได้</p><button id="astroDeep" type="button">ดูเชิงลึก</button>`;$('#astroDeep').onclick=loadDeep;});
  ...
  async function loadDeep(){
    ...
    box.innerHTML='<h2>กำลังอ่านเชิงลึก…</h2><p>กำลังเชื่อมโยงข้อมูลที่คุณบันทึกไว้ในบัญชี</p>';
    try{
